...
from fastapi_app.tasks.dependencies import OwnedTask, owned_task, TaskNotFound

router = APIRouter()


def owned_dacedsx_task(task: OwnedTask = Depends(owned_task)):
    if task.framework != "dacedsx":
        raise TaskNotFound()
    return task
...
def snapshot(request: Request, cursor: int = Query(0, ge=0),
             ...
             task: OwnedTask = Depends(owned_dacedsx_task)):
...
def events(request: Request, cursor: int = Query(0, ge=0), limit: int = Query(100, ge=1, le=1000),
           generation: Optional[str] = None, task: OwnedTask = Depends(owned_dacedsx_task)):
