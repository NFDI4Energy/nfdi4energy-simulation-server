...
class pandapowerAPI:
    ...
    def step(self, step):
        ...
        self.history.append({'step': step, 'converged': converged})
        return converged
    ...
    def get_metric_snapshot(self):
        metrics = {
            "bus.vm_pu.min": None,
            "bus.vm_pu.max": None,
            "line.loading_percent.max": None,
            "load.p_mw.total": None,
        }

        try:
            if hasattr(self.network, "res_bus") and not self.network.res_bus.empty:
                metrics["bus.vm_pu.min"] = round(float(self.network.res_bus["vm_pu"].min()), 4)
                metrics["bus.vm_pu.max"] = round(float(self.network.res_bus["vm_pu"].max()), 4)
        except Exception:
            pass

        try:
            if hasattr(self.network, "res_line") and "loading_percent" in self.network.res_line and not self.network.res_line.empty:
                metrics["line.loading_percent.max"] = round(float(self.network.res_line["loading_percent"].max()), 2)
        except Exception:
            pass

        try:
            if hasattr(self.network, "load") and "p_mw" in self.network.load and not self.network.load.empty:
                metrics["load.p_mw.total"] = round(float(self.network.load["p_mw"].sum()), 4)
        except Exception:
            pass
