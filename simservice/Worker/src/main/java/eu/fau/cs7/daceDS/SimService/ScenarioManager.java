...
public class ScenarioManager {
	...
	// task_id mapping: scenarioID -> task_id (for RabbitMQ-sourced requests)
	private static HashMap<String, String> scenarioToTaskId = new HashMap<String, String>();
	...
	static {
		...
		if (redisHost != null && !redisHost.isEmpty()) {
			try {
				redisClient = new Jedis(redisHost, 6379);
	...
	/** Register a mapping from scenarioID to task_id for Redis status tracking */
	public static void registerTaskId(String scenarioID, String taskId) {
		scenarioToTaskId.put(scenarioID, taskId);
		logger.info("Registered taskId mapping: " + scenarioID + " -> " + taskId);
	}

	/** Update Redis status for a scenario (only if it has a task_id) */
	private static void updateRedisStatus(String scenarioID, String status, Map<String, String> extra) {
		if (redisClient == null) return;
		String taskId = scenarioToTaskId.get(scenarioID);
		...
		try {
			...
			redisClient.hset("task:" + taskId, fields);
			logger.info("Redis: task:" + taskId + " -> " + status);
		} catch (Exception e) {
			logger.error("Redis update failed for task:" + taskId + ": " + e.getMessage());
		}
	}
	...
	public static void killScenario(String scenario) {
		...
		updateRedisStatus(scenario, "ERROR", null);
	}
	...
	public static ArrayList<String> cleanTerminatedScenarios() {
		...
		for(String scenario : newlyTerminated) {
			...
			extra.put("files", "[]"); // TODO: populate with actual result files
			String taskId = scenarioToTaskId.get(scenario);
			SimulationEventEmitter events = new SimulationEventEmitter(taskId, "simservice", "SimService");
			if (r != null && r.hasCorruptedParts()) {
				String error = r.getFailureMessage();
				extra.put("error", error);
				updateRedisStatus(scenario, "ERROR", extra);
				events.emit("ERROR", "TASK_FAILED", error, 1.0, null, null);
			} else {
				updateRedisStatus(scenario, "DONE", extra);
				events.emit("PROGRESS", "TASK_COMPLETED", "Simulation task completed", 1.0, null, null);
			}
		}
	...
	public static int addScenarioRun(Scenario scenario, long timestamp) {
		return addScenarioRun(scenario, timestamp, null);
	}

	public static int addScenarioRun(Scenario scenario, long timestamp, String taskId) {
		...
		ScenarioInstanceExecutor runner = new ScenarioInstanceExecutor(scenario, false, taskId);
		
		//prepare
		if(!runner.instantiateScenario()) {
			logger.info("Init ScenarioRunnerThread failed for " + scenario.getScenarioID());
			Map<String, String> extra = new HashMap<>();
			extra.put("error", "Failed to initialize simulation task");
			updateRedisStatus(scenario.getScenarioID().toString(), "ERROR", extra);
			new SimulationEventEmitter(taskId, "simservice", "SimService").emit("ERROR", "TASK_FAILED", "Failed to initialize simulation task", 1.0, null, null);
			return 3;
		}
		...
		updateRedisStatus(scenario.getScenarioID().toString(), "RUNNING", null);
		new SimulationEventEmitter(taskId, "simservice", "SimService").emit("PROGRESS", "TASK_STARTED", "Simulation task started", 0.20, scenario.getSimulationStart(), null);
